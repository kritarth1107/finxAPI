...
const folioGET = async (req, res) => {
  ...
  var finalsData= [];
  try {
    const { PAN_NO,amc,category } = req.body;
    ...
    if(amc=="" || amc==" " || amc=="all")
    {
        if(category=="" || category==" " || category=="all")
        {
            ...
            .find({ DISTRIBUTOR:key_n[1], PAN_NO: PAN_NO })
            .lean();
        }
        else
        {
            var f = await folioModel
            .find({ DISTRIBUTOR:key_n[1], PAN_NO: PAN_NO, SCH_CATEGORY:category })
            .lean();
        }
    }
    else
    {
        ...
        {
            var f = await folioModel
            .find({ DISTRIBUTOR:key_n[1], PAN_NO: PAN_NO , SCH_AMC:amc , SCH_CATEGORY:category})
            .lean();
        }
        else
        {
            var f = await folioModel
            .find({ DISTRIBUTOR:key_n[1], PAN_NO: PAN_NO, SCH_AMC:amc })
            .lean();
        }
    }
...
const folioClient = async (req,res) =>{
    ...
    try
    {
        ...
        manageService.checkManage(website,invCheck.DISTRIBUTOR,1).then(result=>{
            ...
            return res.status(403).json({ success:false,status:500,message: "Something went Wrong!!" });
        ...
        if(amc=="" || amc==" " || amc=="all")
        {
            ...
            {
                var f = await folioModel
                .find({ DISTRIBUTOR: invCheck.DISTRIBUTOR, PAN_NO: invCheck.PAN_NO  })
                .lean();
            }
            else
            {
                var f = await folioModel
                .find({ DISTRIBUTOR: invCheck.DISTRIBUTOR, PAN_NO: invCheck.PAN_NO, SCH_CATEGORY:category })
                .lean();
            }
        }
        else
        {
            ...
            {
                var f = await folioModel
                .find({ DISTRIBUTOR: invCheck.DISTRIBUTOR, PAN_NO: invCheck.PAN_NO, SCH_AMC:amc , SCH_CATEGORY:category})
                .lean();
            }
            else
            {
                var f = await folioModel
                .find({ DISTRIBUTOR: invCheck.DISTRIBUTOR, PAN_NO: invCheck.PAN_NO, SCH_AMC:amc })
                .lean();
            }
        }
